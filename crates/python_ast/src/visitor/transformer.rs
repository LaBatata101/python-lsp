use crate::{
    ...
    Decorator, ElifElseClause, ExceptHandler, Expr, FString, FStringElement, Keyword, MatchCase,
...
pub trait Transformer {
    ...
    fn visit_expr_context(&self, expr_context: &mut ContextExpr) {
...
pub fn walk_stmt<V: Transformer + ?Sized>(visitor: &V, stmt: &mut Stmt) {
    match stmt {
        ...
        Stmt::Return(ast::ReturnStmt { value, range: _ }) => {
            if let Some(expr) = value {
                visitor.visit_expr(expr);
            }
        }
        Stmt::Delete(ast::DeleteStmt { targets, range: _ }) => {
            for expr in targets {
                visitor.visit_expr(expr);
            }
        }
        ...
        Stmt::With(ast::WithStmt { items, body, .. }) => {
        ...
        Stmt::Import(ast::ImportStmt { names, range: _ }) => {
            for alias in names {
...
pub fn walk_expr<V: Transformer + ?Sized>(visitor: &V, expr: &mut Expr) {
    match expr {
        ...
        Expr::Await(ast::AwaitExpr { value, range: _ }) => visitor.visit_expr(value),
        Expr::Yield(ast::YieldExpr { value, range: _ }) => {
            if let Some(expr) = value {
                visitor.visit_expr(expr);
            }
        }
        Expr::YieldFrom(ast::YieldFromExpr { value, range: _ }) => visitor.visit_expr(value),
        ...
        Expr::FString(ast::FStringExpr { value, .. }) => {
        ...
        Expr::StringLiteral(ast::StringLiteralExpr { value, .. }) => {
            for string_literal in value.iter_mut() {
                visitor.visit_string_literal(string_literal);
            }
        }
        Expr::BytesLiteral(ast::BytesLiteralExpr { value, .. }) => {
        ...
        Expr::Attribute(ast::AttributeExpr { value, ctx, .. }) => {
            visitor.visit_expr(value);
...
pub fn walk_f_string_element<V: Transformer + ?Sized>(
    ...
    if let FStringElement::Expression(ast::FStringExpressionElement {
        expression,
        format_spec,
        ..
    }) = f_string_element
...
pub fn walk_expr_context<V: Transformer + ?Sized>(_visitor: &V, _expr_context: &mut ContextExpr) {}
